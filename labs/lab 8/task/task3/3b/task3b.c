...
void printRelocationTables(state* s, Elf32_Shdr* relTab_header)
{
  ...
  int offset = -1, info = -1, type = -1, value = -1;
  char* name = "";
  for(int i = 0; i < tableSize; i++)
  {
    offset = relocationsTableStart[i].r_offset;
    info = relocationsTableStart[i].r_info;
    type =  ELF32_R_TYPE(relocationsTableStart[i].r_info);
    value = symbolTableStart[ELF32_R_SYM(relocationsTableStart[i].r_info)].st_value;
